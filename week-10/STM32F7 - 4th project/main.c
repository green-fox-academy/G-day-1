...
int main(void) {
	...
	while (1) {
		...
		if (cnt > 25000 && cnt < 60000) {
			...
			HAL_GPIO_WritePin(GPIOG, GPIO_PIN_6, GPIO_PIN_SET);    ///////// JELZÕLÁMPAKÓD
		} else {
		...
		if (cnt3 > 15000 && cnt3 < 25000) {
			...
			HAL_GPIO_WritePin(GPIOB, GPIO_PIN_4, GPIO_PIN_RESET); //sárga
		...
		if (cnt2 < 15000) {
			...
			HAL_GPIO_WritePin(GPIOI, GPIO_PIN_0, GPIO_PIN_RESET); //zöld
		}

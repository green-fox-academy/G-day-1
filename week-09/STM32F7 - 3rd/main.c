...
int main(void) {
	...
	char hello[100] = "Let's play a game! Are you ready? If YES, press the BLUE button.\r\n";
	HAL_StatusTypeDef status = HAL_UART_Transmit(&uart_handle, (uint8_t*) hello, strlen(hello), 0xFFFF);

	state_t program_state = INITIAL_STATE;
	...
	while (1) {
		...
		case INITIAL_STATE:
			...
			if (BSP_PB_GetState(BUTTON_KEY) == 1) {
				...
				printf("OK, look at the LEDs!\r\n");

			}
			break;

		case STATE_1:
			HAL_Delay(random % 10000 + 1);
			HAL_GPIO_WritePin(GPIOF, GPIO_PIN_10, GPIO_PIN_SET);
			BSP_LED_On(LED1);

			start = HAL_GetTick();
			while (BSP_PB_GetState(BUTTON_KEY) == 0); //stop

			end = HAL_GetTick();
			printf("Cool, your reaction time is: %lu milliseconds.\r\n", end - start);
